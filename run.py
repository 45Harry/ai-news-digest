...
Usage:
    python run.py --once                # run the pipeline once and exit (full digest)
    python run.py --breaking-only       # one pass, BREAKING alerts only (no digest)
    python run.py                       # keep running: BREAKING alerts in real time
    python run.py --interval 120        # ... check every 2 minutes (real-time-ish)
...
def _parse_args(argv) -> argparse.Namespace:
    ...
    parser.add_argument("--once", action="store_true", help="run the full pipeline once, then exit")
    parser.add_argument("--breaking-only", action="store_true", help="one pass: BREAKING alerts only, no digest")
